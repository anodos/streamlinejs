...
(function(exports) {
	...
	__g.usetrampoline = true;

	var trampolineCreator = function() {
		var q = [];
		return {
			queue: function(fn) {
				q.push(fn);
	...
	__g.trampoline = trampolineCreator();

	__g.enableTrampoline = function() {
		__g.usetrampoline = true;
		if(!__g.trampoline) __g.trampoline = trampolineCreator();
	};

	__g.disableTrampoline = function() {
		__g.usetrampoline = false;
		__g.trampoline = null;
	};
	...
	function __trap(err) {
      ...
      else if(__g.errorHandler) __g.errorHandler(err);
			else if (typeof process !== 'undefined' && process.nextTick) process.nextTick(function() {
				throw err;
			});
      else {
        console.error('\n======= UNCAUGHT EXCEPTION (and no Streamline errorHandler) =======\n', err.stack, err);
      }
		}
	}
	...
	function __catch(fn) {
		...
		if(__g.trampoline) __g.trampoline.queue(catchcb);
		else catchcb();
	}
	...
	function __setEF(e, f) {
		function formatStack(e, raw) {
			...
			if (s) {
				...
				// Moved these two lines into the if
				// Obviously, if `s` above is falsey, then `raw` will be falsely, and that caused exceptions to be thrown before.
				var nl = raw.indexOf('\n');
